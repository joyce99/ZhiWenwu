...
class Trainer(object):
    ...
    def __init__(self, src_emb, tgt_emb, mapping_G, mapping_F, discriminator_A, discriminator_B, encoder_A, decoder_A, encoder_B, decoder_B, params):
        ...
        self.std_A=0
        self.std_B=0
        self.g_src_emb = np.zeros((200000,0))
        self.g_tgt_emb = np.zeros((200000,0))
    ...
    def mapping_step_G(self, stats):
        ...
        # Total loss
        #1,5,1
        total_loss = loss + self.params.cycle_lambda*cyc_loss + self.params.reconstruction_lambda*loss_A #+ loss_distance
    ...
    """
    def add_noise(self, vector, noise_type="gaussian"):
        row, col = 1, 300
        ...
        if noise_type == "speckle":
            ...
            noise = noise.reshape(row, col)
            vector = vector + vector * noise
            return vector


    def train_autoencoder_A(self):
        self.g_src_emb = np.zeros((len(self.src_dico),300))
        tmp = self.src_emb.weight.data.clone()
        src_emb_numpy = tmp.cpu().detach().numpy()
        print(type(self.src_emb))
        for idx in tqdm(range(100000)):
            ids = torch.LongTensor(idx)
            ids = ids.cuda()
            self.g_src_emb[idx] = src_emb_numpy[idx]
        for idx in tqdm(range(100000,len(self.src_dico))):
            ids = torch.LongTensor(idx)
            ids = ids.cuda()
            self.g_src_emb[idx] = self.add_noise(src_emb_numpy[idx], "gaussian")
        print("Training source in autoencoder.")
        self.g_src_emb = torch.from_numpy(self.g_src_emb).float()
        self.g_src_emb = self.g_src_emb.cuda() if (self.g_src_emb.cuda) else self.g_src_emb
        bs = 128
        for epoch in tqdm(range( self.params.autoenc_epochs)):
            ...
            for n_iter in range(0, epoch_size, bs):
                ...
                g_emb = self.g_src_emb[ids]
                preds = self.decoder_A(self.encoder_A(g_emb.data))
    ...
    def train_autoencoder_B(self):
        self.g_tgt_emb = np.zeros((len(self.tgt_dico), 300))
        tmp = self.tgt_emb.weight.data.clone()
        tgt_emb_numpy = tmp.cpu().detach().numpy()
        print(type(self.tgt_emb))
        for idx in tqdm(range(100000)):
            ids = torch.LongTensor(idx)
            ids = ids.cuda()
            self.g_tgt_emb[idx] = tgt_emb_numpy[idx]
        for idx in tqdm(range(100000,len(self.tgt_dico))):
            ids = torch.LongTensor(idx)
            ids = ids.cuda()
            self.g_tgt_emb[idx] = self.add_noise(tgt_emb_numpy[idx], "gaussian")
        ...
        for epoch in tqdm(range( self.params.autoenc_epochs)):
            ...
            for n_iter in range(0, epoch_size, bs):
                ...
                g_emb = self.g_src_emb[ids]
                preds = self.decoder_B(self.encoder_B(g_emb.data))
                loss = F.mse_loss(emb.data, preds)
                total_loss += loss.detach().item()
                num_batches += 1
                # optim
    ...
    def build_dictionary_AB(self):
        ...
        # for i in range(self.dico_AB.shape[0]):
        #     for j in range(self.dico.shape[0]):
        #         if(self.dico_AB[i][0] == self.dico[j][0]):
        #             self.dico_AB[i][1] = self.dico[j][1]
    ...
    def build_dictionary_BA(self):
        ...
        # for i in range(self.dico_BA.shape[0]):
        #     for j in range(self.dico_B.shape[0]):
        #         if(self.dico_BA[i][0] == self.dico_B[j][0]):
        #             self.dico_BA[i][1] = self.dico_B[j][1]
        self.dico_BA = torch.cat([self.dico_B, self.dico_BA], 0)
    ...
    def save_best_AB(self, to_log, metric_AB):
        ...
        print(self.best_valid_metric_AB)
        print(to_log[metric_AB])
        if to_log[metric_AB] > self.best_valid_metric_AB :
            # new best mapping
