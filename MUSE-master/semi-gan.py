...
parser.add_argument("--cycle_lambda", type=float, default=1, help="Cycle loss feedback coefficient")
...
parser.add_argument("--dis_most_frequent_AB", type=int, default=50000,
                    help="Select embeddings of the k most frequent words for discrimination in source to target (0 to disable)")
parser.add_argument("--dis_most_frequent_BA", type=int, default=50000,
...
# training refinement
parser.add_argument("--n_procrustes", type=int, default=100,
...
parser.add_argument("--l_relu", type=int, default=1,
...
src_emb, tgt_emb, mapping_G, mapping_F, discriminator_A, discriminator_B, encoder_A, decoder_A, encoder_B, decoder_B, discriminator_D = build_model(
    params, True)
trainer = Trainer(src_emb, tgt_emb, mapping_G, mapping_F, discriminator_A, discriminator_B, encoder_A, decoder_A,
                  encoder_B, decoder_B, discriminator_D, params)
...
if params.adversarial:
    ...
    trainer.expectation_A = src_A.mean()  # 2.5916e-06
    trainer.expectation_B = tgt_B.mean()  # -1.6289e-0
    trainer.std_A = src_A.std()  # 0.0526
    trainer.std_B = tgt_B.std()  # 0.0524
    ...
    for n_epoch in range(params.n_epochs):
        ...
        for n_iter in tqdm(range(0, params.epoch_size, params.batch_size)):
            ...
            # trainer.mapping_step_G_KNN(stats)
            # trainer.mapping_step_F_KNN(stats)
            # log stats
            if n_iter % 500 == 0:
        ...
        if trainer.map_optimizer_G.param_groups[0]['lr'] < params.min_lr or trainer.map_optimizer_F.param_groups[0][
            'lr'] < params.min_lr:
